...
def generate_concluding_narrative(
    llm_client: OpenAI, story: Story, next_image: bytes
) -> str:
    """
    Generate the concluding narrative from the given image.
    """

    print(Back.MAGENTA + "GENERATING CONCLUDING NARRATIVE")
    ...
    # Use the image description to generate the concluding narrative
    structured_prompt = StructuredPrompt.from_package_resource(
        package="spooklight.stepgeneration",
        resource_name="generate_concluding_narrative.prompt.md",
    )
    template_values = {
